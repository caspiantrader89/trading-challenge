...
function getRisk(phase) {
  ...
  if (phase === 4)
    return {
      pct: 4,
      cls: "phase4",
      label: "Fase 4 — Conservativa",
      range: "$10.000 → $40.000",
    }
  return {
    pct: 2.5,
    cls: "phase5",
    label: "Fase 5 — Protezione capitale",
...
function setRR(rr) {
  currentRR = rr
  ;[1, 2, 3].forEach((r) =>
    document.getElementById("rr-" + r).classList.toggle("active", r === rr),
  )
  renderTP()
}
function renderTP() {
...
function renderProb() {
  const pct =
    currentState && currentState.trades.length >= 5
      ? calcProbability(currentState)
      : null
  ...
  const rawWr = Math.round(
    (currentState.trades.filter((t) => t.outcome === "win").length /
      currentState.trades.length) *
      100,
  )
  ...
  if (pct >= 50) {
    ...
    detail =
      "Win rate reale " +
      rawWr +
      "% — traiettoria positiva. Mantieni la disciplina."
  } else if (pct >= 20) {
    color = "var(--phase2)"
    verdict = "Media"
    detail =
      "Win rate reale " +
      rawWr +
      "% — possibilità reali ma ogni trade è determinante."
  } else if (pct >= 8) {
    color = "var(--loss)"
    verdict = "Bassa"
    detail =
      "Win rate reale " + rawWr + "% — percorso difficile. Rivedi la strategia."
  } else {
    color = "var(--loss)"
    verdict = "Critica"
    detail =
      "Win rate reale " +
      rawWr +
      "% — matematicamente quasi impossibile. Stop e analisi."
  }
  descEl.innerHTML =
    '<strong style="color:' +
    color +
...
function renderState(state) {
  ...
  document.getElementById("risk-amt").textContent =
    "stop loss: " + fmt((cap * risk.pct) / 100)
  const _sl = (cap * risk.pct) / 100
  const _tp = _sl * 2
  if (document.getElementById("auto-win-sub"))
    document.getElementById("auto-win-sub").textContent =
      "+" + fmt(_tp) + " calcolato auto"
  if (document.getElementById("auto-loss-sub"))
    document.getElementById("auto-loss-sub").textContent =
      "\u2212" + fmt(_sl) + " calcolato auto"
  ...
  document.getElementById("s-winrate").className =
    "sv " + (total > 0 && wins / total >= 0.5 ? "pos" : "neg")
  document.getElementById("s-profit").textContent =
    total > 0 ? (totalPnl >= 0 ? "+" : "−") + fmt(Math.abs(totalPnl)) : "—"
  document.getElementById("s-profit").className =
    "sv " + (totalPnl >= 0 ? "pos" : "neg")
  document.getElementById("s-best").textContent =
    best !== null ? (best >= 0 ? "+" : "−") + fmt(Math.abs(best)) : "—"
  document.getElementById("s-best").className =
    "sv " + (best !== null && best >= 0 ? "pos" : "neg")
  const list = document.getElementById("trade-list")
  if (!state.trades.length) {
    list.innerHTML = '<div class="empty">Nessun trade registrato</div>'
  } else {
    list.innerHTML = [...state.trades]
      .reverse()
      .map((t, i) => {
        const num = state.trades.length - i
        const sign = t.pnl >= 0 ? "+" : "−"
        const symTag = t.symbol
          ? `<span style="font-size:10px;color:var(--text-muted);margin-right:4px">${t.symbol}</span>`
          : ""
        const modeTag =
          t.mode === "bitget"
            ? `<span style="font-size:9px;padding:1px 5px;border-radius:2px;background:rgba(0,212,170,0.1);color:#00d4aa;margin-left:3px;letter-spacing:0.05em">Bitget</span>`
            : ""
...
function renderLeaderboard(entries, myUsername) {
  ...
  if (!list) return
  if (!entries.length) {
    list.innerHTML = '<div class="empty">Nessun trader ancora.</div>'
    return
  }
  ...
  list.innerHTML = sorted
    .map((e, i) => {
      ...
      return (
        '<div style="display:grid;grid-template-columns:36px 1fr auto auto;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid var(--border)">' +
        '<span style="font-size:15px;text-align:center">' +
        medal +
        "</span>" +
        '<a href="?u=' +
        encodeURIComponent(e.username) +
        '" target="_blank" style="font-size:13px;font-weight:500;color:' +
        (isMe ? "var(--accent)" : "var(--text)") +
        ';text-decoration:none;">' +
        e.username +
        (isMe ? " (tu)" : "") +
        "</a>" +
        '<span style="font-size:10px;padding:2px 8px;border-radius:3px;background:' +
        phaseBgs[phase] +
        ";color:" +
        phaseColors[phase] +
        ';text-transform:uppercase;letter-spacing:0.06em">F' +
        phase +
        "</span>" +
        '<span style="font-family:var(--display);font-weight:700;font-size:14px">' +
        fmt(e.capital) +
        "</span>" +
        "</div>"
      )
...
function showApp(state, readOnly = false) {
  ...
  if (!appInitialized) {
    ...
    document.getElementById("share-btn").style.display = readOnly
      ? "none"
      : "inline-block"
    document.getElementById("journal-btn").style.display = readOnly
      ? "none"
      : "inline-block"
    document.getElementById("profile-btn").style.display = readOnly
      ? "none"
      : "inline-flex"
    document.getElementById("trade-form-section").style.display = readOnly
      ? "none"
      : "block"
    if (readOnly) {
    ...
    if (!readOnly)
      syncKeysFromFirestore()
        .then(() => initBitgetDashboard())
        .catch(() => {})
    if (unsubLb) unsubLb()
    unsubLb = onSnapshot(collection(db, "challenges"), (snap) => {
      const entries = snap.docs
        .map((d) => d.data())
        .filter((d) => d.username && d.capital !== undefined)
...
window.switchTab = function (tab) {
  ;["login", "register", "forgot"].forEach((t) => {
    ...
    document
      ...
      [
        ["login", "register", "forgot"].indexOf(t)
...
window.doLogin = async function (event) {
  ...
  if (!token) {
    document.getElementById("login-err").textContent =
      "Attendi la verifica di sicurezza."
    return
  }

  try {
    const captchaRes = await fetch(
      "https://vanillachart-api.vercel.app/api/login",
      {
        method: "POST",
    ...
    if (!captchaRes.ok) {
      document.getElementById("login-err").textContent =
        captchaResult.error || "Verifica captcha fallita."
      return
    }
  } catch (e) {
    document.getElementById("login-err").textContent =
  ...
  if (!user || !pass) {
    document.getElementById("login-err").textContent = "Compila tutti i campi."
    return
  }
  ...
  try {
    ...
    if (snap.empty) {
      document.getElementById("login-err").textContent = "Username non trovato."
      btn.disabled = false
      return
    }
...
window.doRegister = async function (event) {
  ...
  if (!FIREBASE_CONFIGURED) {
    document.getElementById("reg-err").textContent =
      "Firebase non configurato — vedi istruzioni."
    return
  }

  // 1. Captcha check
  const tokenInput = document.querySelector(
    '#form-register [name="cf-turnstile-response"]',
  )
  const token = tokenInput ? tokenInput.value : null
  if (!token) {
    document.getElementById("reg-err").textContent =
  ...
  try {
    const captchaRes = await fetch(
      "https://vanillachart-api.vercel.app/api/register",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ turnstileToken: token }),
  ...
  if (!user || !email || !pass) {
    document.getElementById("reg-err").textContent = "Compila tutti i campi."
    return
  }
  if (user.length < 3) {
    document.getElementById("reg-err").textContent =
  ...
  try {
    ...
    if (unameDoc.exists()) {
      document.getElementById("reg-err").textContent = "Username già in uso."
      btn.disabled = false
      return
    }
...
window.doForgot = async function (event) {
  ...
  // 1. Captcha check
  const tokenInput = document.querySelector(
    '#form-forgot [name="cf-turnstile-response"]',
  )
  const token = tokenInput ? tokenInput.value : null
  if (!token) {
    document.getElementById("forgot-err").textContent =
  ...
  try {
    const captchaRes = await fetch(
      "https://vanillachart-api.vercel.app/api/forgot",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ turnstileToken: token }),
...
async function saveState(ns) {
  await setDoc(doc(db, "challenges", currentUser.uid), ns)
}
...
window.openShareModal = function () {
  ...
  const url =
    ...
    "?u=" +
    encodeURIComponent(currentState.username)
...
async function init() {
  ...
  if (sharedUser) {
    ...
    try {
      const q = query(
        collection(db, "usernames"),
        where("username", "==", sharedUser.toLowerCase()),
      )
      const snap = await getDocs(q)
      if (!snap.empty) {
        ...
        if (challengeDoc.exists()) {
          ...
          onSnapshot(doc(db, "challenges", uid), (d) => {
            if (d.exists()) renderState(d.data())
          })
          onSnapshot(doc(db, "bitgetPublic", uid), (d) => {
            if (d.exists()) renderReadonlyBitgetDashboard(d.data())
          })
          return
        }
      }
    ...
    document.getElementById("loading-screen").innerHTML =
      '<div class="loading-text" style="color:var(--text-muted)">Challenge non trovata.</div>'
    return
  }
...
window.toggleTP = function (n) {
  ...
  if (tpEnabled[idx]) {
    ...
    for (let i = n; i < 3; i++) {
      ...
      document.getElementById("toggle-tp" + (i + 1)).textContent =
        "+ TP" + (i + 1)
...
window.calcJournalPnl = function () {
  ...
  ;["tp1-preview", "tp2-preview", "tp3-preview", "exit-preview"].forEach(
    (id) => {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const tpPrice = parseFloat(
      document.getElementById("jf-tp" + n + "-price").value,
    )
    const tpPct = parseFloat(
      document.getElementById("jf-tp" + n + "-pct").value,
    )
    if (isNaN(tpPrice) || isNaN(tpPct) || tpPct <= 0) {
      hasEnoughData = false
      break
    }
    ...
    totalPnl += pPnl
    weightedRR += (diff / slDist) * closed
    totalWeight += closed
    remainingSize -= closed
    const resEl = document.getElementById("tp" + n + "-residuo")
    if (resEl)
      resEl.textContent = "(residuo: $" + remainingSize.toFixed(2) + ")"
    const prevEl = document.getElementById("tp" + n + "-preview")
    if (prevEl)
      prevEl.innerHTML = `<span style="color:${pPnl >= 0 ? "var(--win)" : "var(--loss)"};font-weight:600">${pPnl >= 0 ? "+" : ""}$${pPnl.toFixed(2)}</span><br>Chiudi $${closed.toFixed(2)}`
  }

  document.getElementById("jf-residuo-label").textContent =
    remainingSize < size ? "(residuo: $" + remainingSize.toFixed(2) + ")" : ""

  if (!isNaN(exit) && remainingSize > 0) {
    const diff = isLong ? exit - entry : entry - exit
    const fPnl = (diff / entry) * remainingSize
    totalPnl += fPnl
    weightedRR += (diff / slDist) * remainingSize
    totalWeight += remainingSize
    const ep = document.getElementById("exit-preview")
    if (ep)
      ep.innerHTML = `<span style="color:${fPnl >= 0 ? "var(--win)" : "var(--loss)"};font-weight:600">${fPnl >= 0 ? "+" : ""}$${fPnl.toFixed(2)}</span><br>Chiudi $${remainingSize.toFixed(2)}`
...
window.saveJournalEntry = async function () {
  ...
  if (!symbol) {
    alert("Inserisci il Pair.")
    btn.disabled = false
    btn.textContent = "+ Aggiungi al Journal"
    return
  }
  if (isNaN(entryP) || isNaN(slP) || isNaN(size) || size <= 0) {
    alert("Inserisci Entry, SL e Size.")
    btn.disabled = false
    btn.textContent = "+ Aggiungi al Journal"
    return
  }
  if (isNaN(exitP)) {
    alert("Inserisci il prezzo di uscita finale.")
    btn.disabled = false
    btn.textContent = "+ Aggiungi al Journal"
    return
  }
  ...
  for (let i = 0; i < 3; i++) {
    ...
    if (isNaN(tpPrice) || isNaN(tpPct) || tpPct <= 0) {
      alert("Compila prezzo e % per TP" + n + ".")
      btn.disabled = false
      btn.textContent = "+ Aggiungi al Journal"
      return
    }
    ...
    tps.push({
      price: tpPrice,
      pct: tpPct,
      closedSize: parseFloat(closed.toFixed(4)),
      pnl: parseFloat(pPnl.toFixed(4)),
    })
    remainingSize -= closed
  }
  ...
  totalPnl += finalPnl
  weightedRR += (finalDiff / slDist) * remainingSize
  totalWeight += remainingSize

  const avgRR =
    totalWeight > 0 ? parseFloat((weightedRR / totalWeight).toFixed(2)) : null
  ...
  const entryObj = {
    ...
    tps,
    pnlIsR: false,
    mood: document.getElementById("jf-mood")?.value || "",
    screenshot: screenshotUrl,
  }
  await setDoc(doc(db, "journals", currentUser.uid), {
    entries: [entryObj, ...journalEntries],
  })
  ;[
    "jf-symbol",
    "jf-entry",
  ...
  document.getElementById("jf-dir").value = "long"
  document.getElementById("jf-tf").value = ""
  document.getElementById("jf-calc-row").style.display = "none"
  ...
  document.getElementById("jf-mood").value = ""
  screenshotB64 = ""
  screenshotFile = null
  document
    .querySelectorAll("#journal-overlay .mood-btn")
  ...
  const sz = document.getElementById("screenshot-zone")
  if (sz) sz.classList.remove("has-img")
...
window.deleteJournalEntry = async function (id) {
  ...
  await setDoc(doc(db, "journals", currentUser.uid), {
    entries: journalEntries.filter((e) => e.id !== id),
  })
}
...
window.saveEditJournal = async function () {
  ...
  const updated = journalEntries.map((e) => {
    ...
    return {
      ...
      symbol:
        document.getElementById("ej-symbol").value.trim().toUpperCase() ||
        e.symbol,
      dir: document.getElementById("ej-dir").value,
      entryPrice:
        parseFloat(document.getElementById("ej-entry").value) || e.entryPrice,
      slPrice: parseFloat(document.getElementById("ej-sl").value) || e.slPrice,
      exitPrice:
        parseFloat(document.getElementById("ej-exit").value) || e.exitPrice,
...
window.confirmResetJournal = async function () {
  ...
  if (
    !confirm(
      "Sei sicuro? Tutti i trade del journal verranno eliminati definitivamente.",
    )
  )
    return
...
async function fetchAllBitgetFills(sinceTimestamp = null) {
  ...
  while (page < MAX_PAGES) {
    ...
    const timestamps = fills
      .map((f) => parseInt(f.cTime || f.createTime || f.updateTime || 0))
      .filter(Boolean)
...
window.importBitgetToJournal = async function () {
  ...
  if (!apiKey || !secret) {
    alert("API Bitget non configurate. Configurale nel profilo prima.")
    return
  }
  ...
  try {
    ...
    const importedEntries = journalEntries.filter(
      (e) => e.bitgetOrderId && e.lastFillTs,
    )
    const lastTs =
    ...
    const importedJournalIds = new Set(
      journalEntries.map((e) => e.bitgetOrderId).filter(Boolean),
    )
    ...
    for (const [orderId, fills] of orderMap) {
      ...
      const totalPnl = fills.reduce(
        (s, o) => s + parseFloat(o.profit || o.pnl || o.realizedPnl || 0),
        0,
      )
      const sym =
        (first.symbol || "")
      ...
      const ts = parseInt(
        first.cTime || first.createTime || first.updateTime || Date.now(),
      )
      const dateStr = new Date(ts).toISOString().split("T")[0]
      const entryPrice =
        parseFloat(
          first.price || first.fillPrice || first.avgEntryPrice || 0,
        ) || null
      const size =
        parseFloat(first.baseVolume || first.size || first.vol || 0) || null
    ...
    // Salva — nuovi in cima
    await setDoc(doc(db, "journals", currentUser.uid), {
      entries: [...newEntries, ...journalEntries],
    })
    ...
    if (btn) {
      btn.disabled = false
      btn.textContent = "⬇ Importa Bitget"
    }
  }
}
...
function drawPieChart(wins, losses, bes) {
  const canvas = document.getElementById("journal-pie")
  if (!canvas) return
  ...
  if (total === 0) {
    ...
    ctx.fill()
    document.getElementById("journal-pie-legend").textContent = "Nessun dato"
    return
  }
  const slices = [
    { val: wins, color: "#3d9e60" },
    { val: losses, color: "#e05a2b" },
    { val: bes, color: "#5b8cf7" },
  ].filter((s) => s.val > 0)
  let a = -Math.PI / 2
  slices.forEach((s) => {
  ...
  const leg = [`✅ ${wins}W`, `❌ ${losses}L`]
  if (bes > 0) leg.push(`➖ ${bes}BE`)
...
window.renderJournal = function () {
  ...
  wrEl.className =
    "jval " + (wrPct !== null ? (wrPct >= 50 ? "pos" : "neg") : "neutral")
  ...
  document.getElementById("jst-best").textContent =
    best !== null ? fmtPnl(best) : "—"
  document.getElementById("jst-rr").textContent = avgRR ? avgRR + ":1" : "—"
  if (typeof drawPieChart === "function") drawPieChart(wins, losses, bes)
  if (typeof renderJournalAdvStats === "function")
    renderJournalAdvStats(entries)

  const setupMap = {}
  entries.forEach((e) => {
    if (!e.setup) return
    if (!setupMap[e.setup]) setupMap[e.setup] = { wins: 0, total: 0, pnl: 0 }
    setupMap[e.setup].total++
    if (e.outcome === "win") setupMap[e.setup].wins++
  ...
  if (lbEl)
    lbEl.innerHTML =
      setups.length === 0
        ? '<div class="empty" style="padding:1rem">Aggiungi trade con un setup per vedere la classifica.</div>'
        : setups
            .map(([name, s], i) => {
              const medal =
                i === 0 ? "🥇" : i === 1 ? "🥈" : i === 2 ? "🥉" : "#" + (i + 1)
              const wr = Math.round((s.wins / s.total) * 100)
              const pc = s.pnl >= 0 ? "var(--win)" : "var(--loss)"
  ...
  tbody.innerHTML = entries
    .map((e, i) => {
      ...
      let outBg = "rgba(149, 165, 166, 0.15)",
        outCol = "#95a5a6"
      if (e.outcome === "win") {
        outBg = "rgba(46, 204, 113, 0.15)"
        outCol = "#2ecc71"
      }
      if (e.outcome === "loss") {
        outBg = "rgba(231, 76, 60, 0.15)"
        outCol = "#e74c3c"
      }
...
window.selectChMood = function (btn) {
  document
    .querySelectorAll("#ch-mood-row .mood-btn")
    .forEach((b) => b.classList.remove("active"))
  btn.classList.add("active")
...
async function uploadScreenshot(entryId) {
  ...
  try {
    ...
    const storageRef = ref(
      storage,
      `screenshots/${currentUser.uid}/${entryId}.${ext}`,
    )
...
function calcAdvStats(trades) {
  ...
  const avgLoss =
    losses.length > 0 ? (totalLoss / losses.length).toFixed(2) : null
  const exp =
    avgWin && avgLoss && trades.length > 0
      ? (
          (wins.length / trades.length) * parseFloat(avgWin) -
          (losses.length / trades.length) * parseFloat(avgLoss)
        ).toFixed(2)
      : null
  // streaks
  let maxWs = 0,
  ...
  const avgRRWin =
    rrWins.length > 0
      ? (rrWins.reduce((a, b) => a + b, 0) / rrWins.length).toFixed(2)
      : null
  return {
    pf,
    avgWin,
...
function updateChStats(trades) {
  ...
  set(
    ...
    s.exp
      ? parseFloat(s.exp) >= 0
        ...
        : "-$" + Math.abs(s.exp)
      : "—",
    s.exp && parseFloat(s.exp) >= 0 ? "pos" : "neg",
  )
  ...
  ;["ms-500", "ms-2k", "ms-10k", "ms-40k", "ms-100k"].forEach((id, i) => {
    ...
    const el = document.getElementById(id)
    if (el) el.classList.toggle("achieved", cap >= targets[i])
  })
  // 🎉 100k celebration
  ...
  if (
    cap <= 0 &&
    trades.length > 0 &&
    !document.getElementById("challenge-lost-overlay")
  ) {
    launchLoss()
  }
}
...
function renderMoodAnalysis(entries) {
  const tbody = document.getElementById("mood-analysis-tbody")
  if (!tbody) return
  const moods = {
    sicuro: "🟢 Sicuro",
    neutro: "⚪ Neutro",
  ...
  entries.forEach((e) => {
    ...
    map[m].total++
    if (e.outcome === "win") map[m].wins++
    map[m].pnl += e.pnl
    if (e.rr) map[m].rr.push(e.rr)
  })
  const rows = Object.entries(map)
  if (!rows.length) {
    tbody.innerHTML =
      '<tr><td colspan="5" style="color:var(--text-dim);font-size:11px;padding:1rem">Aggiungi trade con stato mentale per vedere l&#39;analisi.</td></tr>'
    return
  }
  tbody.innerHTML = rows
    .map(([m, s]) => {
      ...
      const avgRR =
        s.rr.length > 0
          ? (s.rr.reduce((a, b) => a + b, 0) / s.rr.length).toFixed(2)
          : "-"
...
function renderLeaderboardByMode(entries, myUsername) {
  ...
  let sorted
  if (lbMode === "capitale")
    sorted = [...entries].sort((a, b) => b.capital - a.capital)
  else if (lbMode === "winrate")
    sorted = [...entries]
      .filter((e) => e.trades && e.trades.length >= 3)
      .sort((a, b) => {
        const wrA =
          a.trades.filter((t) => t.outcome === "win").length / a.trades.length
        const wrB =
          b.trades.filter((t) => t.outcome === "win").length / b.trades.length
        return wrB - wrA
      })
  else if (lbMode === "disciplina")
    sorted = [...entries].sort((a, b) => {
      const lsA = a.consecutiveLoss || 0,
        lsB = b.consecutiveLoss || 0
      return lsA - lsB
    })
  else if (lbMode === "streak")
    sorted = [...entries].sort((a, b) => {
      const lsA = a.consecutiveLoss || 0,
        lsB = b.consecutiveLoss || 0
      return lsB - lsA
  ...
  list.innerHTML = sorted
    .map((e, i) => {
      ...
      if (lbMode === "winrate" && e.trades && e.trades.length > 0) {
        const wr = Math.round(
          (e.trades.filter((t) => t.outcome === "win").length /
            e.trades.length) *
            100,
        )
      ...
      return (
        ...
        (e.avatarUrl
          ? `<img src="${e.avatarUrl}" style="width:26px;height:26px;border-radius:50%;object-fit:cover;border:1px solid var(--border-hover);flex-shrink:0" onerror="this.style.display='none'"/>`
          : '<span style="width:26px;height:26px;border-radius:50%;background:var(--surface2);border:1px solid var(--border);display:inline-flex;align-items:center;justify-content:center;font-size:12px;flex-shrink:0">👤</span>') +
        '<a href="?u=' +
        encodeURIComponent(e.username) +
        '" target="_blank" style="font-size:13px;font-weight:500;color:' +
        (isMe ? "var(--accent)" : "var(--text)") +
...
function renderJournalAdvStats(entries) {
  ...
  const exp =
    avgWin && avgLoss && total > 0
      ? (
          (wins.length / total) * parseFloat(avgWin) -
          (losses.length / total) * parseFloat(avgLoss)
        ).toFixed(2)
      : null
  const today = new Date().toISOString().split("T")[0]
  ...
  set(
    "jst-exp",
    exp ? (parseFloat(exp) >= 0 ? "+$" + exp : "-$" + Math.abs(exp)) : "—",
    exp && parseFloat(exp) >= 0 ? "pos" : "neg",
  )
  ...
  if (jCool) {
    ...
    entries.forEach((e) => {
      if (e.outcome === "loss") {
        consLossJ++
        if (consLossJ > maxCl) maxCl = consLossJ
      } else consLossJ = 0
    })
...
async function loadUserProfile() {
  ...
  try {
    const currentUsername = currentState?.username || userProfile.username
    if (currentUsername) {
      const unameSnap = await getDoc(doc(db, "usernames", currentUsername))
      if (unameSnap.exists()) {
        const hasConsent = unameSnap.data().hasConsent || false
        userProfile.hasConsent = hasConsent
        const checkbox = document.getElementById("profile-commercial-checkbox")
        if (checkbox) checkbox.checked = hasConsent
      }
    }
  } catch (e) {}
}
...
window.openProfile = async function () {
  ...
  ;["profile-cur-pass", "profile-new-pass", "profile-confirm-pass"].forEach(
    (id) => {
      const el = document.getElementById(id)
      if (el) el.value = ""
    },
...
window.handleCommercialConsent = async function (isGranted) {
  ...
  const currentUsername =
    typeof currentState !== "undefined" && currentState?.username
...
window.handleProfilePic = async function (input) {
  ...
  if (uploadingEl) {
    uploadingEl.textContent = "Caricamento in corso..."
  ...
  try {
    ...
    updateHeaderAvatar()
    if (uploadingEl) {
      uploadingEl.textContent = "✅ Foto caricata!"
      setTimeout(() => {
    ...
    if (uploadingEl) {
      uploadingEl.textContent = "❌ Errore nel caricamento."
      setTimeout(() => {
        uploadingEl.style.display = "none"
      }, 3000)
    }
...
window.changePassword = async function () {
  ...
  if (newPass.length < 6) {
    msgEl.textContent = "La nuova password deve essere di almeno 6 caratteri."
    msgEl.className = "profile-msg err"
    return
  }
  if (newPass !== confPass) {
...
window.openCookieSettings = function () {
  ...
  if (analyticsEl)
    analyticsEl.checked = localStorage.getItem("gdpr_analytics") === "true"
...
window.deleteUserAccount = async function () {
  ...
  const confirmDialog = confirm(
    "Sei sicuro al 100%? Questa azione è irreversibile.",
  )
...
window.setCalcMarket = function (market, btn) {
  ...
  document
    ...
    .forEach((b) => b.classList.remove("active"))
  if (btn) btn.classList.add("active")
  document.getElementById("calc-crypto-fields").style.display =
    market === "crypto" ? "block" : "none"
  document.getElementById("calc-forex-fields").style.display =
    market === "forex" ? "block" : "none"
  document.getElementById("calc-indici-fields").style.display =
    market === "indici" ? "block" : "none"
  calcRisk()
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (indSel) {
    indSel.addEventListener("change", () => {
      ...
      if (wrap)
        wrap.style.display = indSel.value === "custom" ? "block" : "none"
    })
  }
})
...
window.calcRisk = function () {
  ...
  if (isNaN(entry) || isNaN(sl) || entry <= 0 || sl <= 0 || entry === sl) {
    ...
    if (badge) {
      badge.classList.remove("visible", "long", "short")
    }
    return
  }
  warnEl.classList.remove("visible")
  ...
  if (calcMarket === "crypto") {
    ...
    const maxLev =
      parseFloat(document.getElementById("calc-crypto-maxlev").value) || 50
    sizeLabel = "Size (USDT)"
    ...
    if (leva > maxLev)
      warning = `⚠️ Leva ${leva.toFixed(1)}x supera il massimo del broker (${maxLev}x). Riduci la size o allarga lo SL.`
    ...
    if (lots < 0.01)
      warning =
        "⚠️ Size troppo piccola (<0.01 lot). SL troppo largo o capitale insufficiente."
  } else if (calcMarket === "indici") {
    const indice = document.getElementById("calc-indice").value
    let pointVal = INDICE_POINT[indice]
    if (indice === "custom")
      pointVal =
        parseFloat(document.getElementById("calc-indice-custom").value) || null
    if (!pointVal) {
      resultsEl.style.display = "none"
      return
    }
    ...
    if (contracts < 0.001)
      warning = "⚠️ Size troppo piccola. SL troppo largo rispetto al capitale."
  }

  resultsEl.style.display = "grid"
  document.getElementById("cr-rischio").textContent = "$" + riskUSD.toFixed(2)
  document.getElementById("cr-rischio-pct").textContent =
    risk.pct + "% · Fase " + phase + " · " + dirLabel
  ...
  if (badge) {
    ...
    document.getElementById("calc-dir-label").textContent = isLong
      ? "Long"
      : "Short"
  }
...
async function syncKeysFromFirestore() {
  ...
  if (!localStorage.getItem("bitget_api_key")) {
    ...
    if (keys.passphrase)
      localStorage.setItem("bitget_api_passphrase", keys.passphrase)
...
function updateBitgetUI(state) {
  ...
  if (state === "connected") {
    ...
    if (hdrDot) {
      hdrDot.className = "bitget-hdr-dot connected"
    }
...
window.saveBitgetApiKeys = async function () {
  ...
  const passphrase = (
    document.getElementById("bitget-api-passphrase-input")?.value || ""
  ).trim()
  ...
  try {
    ...
    msg.textContent =
      "✗ " + e.message + " — Verifica le chiavi e i permessi di lettura."
    msg.className = "bitget-msg err"
...
window.disconnectBitget = async function () {
  ...
  if (msg) {
    msg.textContent = "API Bitget disconnessa."
    msg.className = "bitget-msg"
  }
...
window.fetchBitgetData = async function () {
  ...
  try {
    ...
    const balanceData = await bitgetFuturesRequest(
    ...
    // 2. Posizioni aperte
    const posData = await bitgetFuturesRequest(
      "/api/v2/mix/position/all-position",
      { productType: "USDT-FUTURES", marginCoin: "USDT" },
    )
    const positions = (posData.data || []).filter(
      (p) => parseFloat(p.total || p.openDelegateSize || p.available || 0) > 0,
    )
    document.getElementById("bitget-open-positions").textContent =
      positions.length
    ...
    try {
      ...
      orders =
        histData.data?.fillList ||
        histData.data?.orderList ||
        histData.data ||
        []
    } catch (histErr) {
...
async function fetchBitgetDataV2() {
  throw new Error(
    "Endpoint non disponibile. Verifica API Key e Passphrase Bitget.",
  )
}
...
window.bitgetApplyBalance = async function () {
  ...
  if (isReadOnly) {
    alert("Modalità sola lettura.")
    return
  }
  if (!currentState) {
    alert("Stato challenge non caricato, riprova.")
    return
  }

  const balEl = document.getElementById("bitget-dash-balance")
  const normalized = balEl ? balEl.getAttribute("data-raw") : null
  if (!balEl || normalized === null) {
    alert("Saldo Bitget non ancora caricato, attendi il sync.")
    return
  }

  const val = parseFloat(normalized)
  if (isNaN(val) || val < 0) {
    alert("Valore non valido: " + normalized)
    return
  }
...
async function fetchAndRenderDashboard() {
  ...
  try {
    // 1. Balance
    const balData = await bitgetFuturesRequest("/api/v2/mix/account/accounts", {
      productType: "USDT-FUTURES",
    })
    let rawBal = balData.data || []
    if (!Array.isArray(rawBal)) rawBal = [rawBal]
    usdt =
      rawBal.find((a) => (a.marginCoin || "").toUpperCase() === "USDT") ||
      rawBal[0]
    ...
    if (usdt) {
      ...
      if (upnlEl) {
        ...
        upnlEl.className =
          "bdm-val " + (upnl > 0 ? "pnl-pos" : upnl < 0 ? "pnl-neg" : "")
      }
      ...
      if (currentUser && !isReadOnly && currentState && equity > 0) {
        if (Math.abs(equity - currentState.capital) >= 0.01) {
          try {
            await saveState(
              Object.assign({}, currentState, { capital: equity }),
            )
          } catch (saveErr) {
    ...
    const positions = (posData.data || []).filter(
      (p) => parseFloat(p.total || p.available || 0) > 0,
    )
      ...
      try {
        ...
        const posSnap = positions.map((p) => ({
          ...
          unrealizedPL: parseFloat(
            p.unrealizedPL || p.unrealizedProfitLoss || 0,
          ),
          leverage: p.leverage || "",
        ...
        console.warn("snapshot save error:", snapErr.message)
      }
    }
...
async function importBitgetTradesIntoChallenge(orders) {
  ...
  const closeFills = orders.filter((o) => {
    const tradeSide = o.tradeSide || o.side || ""
    const isClose =
      tradeSide.startsWith("close") ||
      tradeSide === "sell" ||
      tradeSide === "buy_to_cover"
    if (!isClose) return false
  ...
  for (const [orderId, group] of orderMap) {
    ...
    const totalPnl = fills.reduce(
      (sum, o) => sum + parseFloat(o.profit || o.pnl || o.realizedPnl || 0),
      0,
    )
    ...
    const dateStr = ts
      ? new Date(parseInt(ts)).toLocaleDateString("it-IT", {
          day: "2-digit",
          month: "2-digit",
        }) +
        " " +
        new Date(parseInt(ts)).toLocaleTimeString("it-IT", {
          hour: "2-digit",
          minute: "2-digit",
        })
      : getTimestamp()
...
function renderReadonlyBitgetDashboard(snap) {
  ...
  if (balEl)
    balEl.textContent =
      "$" +
      parseFloat(snap.equity || 0).toLocaleString("it-IT", {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      })
  const upnlEl = document.getElementById("bitget-dash-upnl")
  if (upnlEl) {
    const u = parseFloat(snap.upnl || 0)
    upnlEl.textContent = (u >= 0 ? "+" : "") + "$" + u.toFixed(2)
    upnlEl.className = "bdm-val " + (u > 0 ? "pnl-pos" : u < 0 ? "pnl-neg" : "")
  }
  const upnlSub = document.getElementById("bitget-dash-upnl-sub")
  if (upnlSub) {
    const u = parseFloat(snap.upnl || 0)
  ...
  const timeEl = document.getElementById("bitget-sync-time")
  if (timeEl) {
    const d = new Date(snap.syncedAt)
    timeEl.textContent =
      "Snapshot " +
