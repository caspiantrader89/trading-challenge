...
function getLadderWeights(n, prices, orders){
  ...
  // Pesi scalari predefiniti: L1 parte da 15% e scala verso il basso
  // Più ordini ci sono, più il peso si distribuisce crescendo verso l'ultimo
  ...
  for(let i=0;i<n;i++){
    ...
    <div class="ladd-order ${enabled?'active':'inactive'}" id="ladd-row-${i}">
      ...
      <div style="display:flex;align-items:center;background:var(--surface3);border:1px solid var(--border2);border-radius:3px;height:22px;min-width:52px">
        <input type="number" id="ladd-pct-${i}" placeholder="auto" min="1" max="99" step="1"
          value="${customPct}"
...
function calcLadder(){
  ...
  for(let i=0;i<n;i++){
    ...
    if(!od){
      if(pc && document.activeElement !== pc) pc.placeholder='auto';
      if(sz) sz.textContent='— USDT';
      continue;
    }
