  ...
  for(let i=0;i<n;i++){
    ...
    <div class="ladd-order ${enabled?'active':'inactive'}" id="ladd-row-${i}">
      ...
      <div class="ladd-price-wrap" style="border-color:${LADDER_COLORS[i]}44">
        <input type="number" id="ladd-price-${i}" placeholder="${i===0?'Top':i===n-1?'Bottom':'prezzo'}"
...
function calcLadder(){
  ...
  // Calcola pesi: usa manuali se disponibili, altrimenti default L1=15% crescente
  const prices = activeOrders.map(o=>o.price);
  const manualPcts = activeOrders.map(o => o.manualPct);
  const hasManual = manualPcts.some(p => p != null);
  const weightsToUse = hasManual ? manualPcts.map((p,i) => p != null ? p : 0) : null;
  const weights = getLadderWeights(activeOrders.length, prices, weightsToUse);
