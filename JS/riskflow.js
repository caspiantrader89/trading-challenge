...
chartEl.addEventListener('mousedown', e=>{
  ...
  if(posLine){
    ...
    window._posDragState = { ...posLine, startY: e.clientY, pending: true };
    return;
  }
...
(async () => {
  ...
  const { getFirestore, doc, setDoc, getDoc, deleteDoc, collection, query, where, getDocs, onSnapshot } = await import('https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js');
...
// ── TRADING JOURNAL — RiskFlow Integration ──────────────────────────────
// ═══════════════════════════════════════════════════════════════════════════

// ── Journal state ──
let rfJournalEntries = [];
let _unsubJournal = null;
let _rfTpEnabled  = [false, false, false];
let _rfScreenshotFile = null;
let _rfScreenshotB64  = '';
...
window.openJournal = async function() {
  ...
  const symEl = document.getElementById('symInput');
  const jfSym = document.getElementById('jf-symbol');
  if (jfSym && symEl && !jfSym.value) jfSym.value = symEl.textContent || '';
  // Subscribe to journal Firestore
  if (_unsubJournal) _unsubJournal();
  _unsubJournal = onSnapshot(doc(db, 'journals', currentUser.uid), snap => {
    rfJournalEntries = snap.exists() ? (snap.data().entries || []) : [];
    rfRenderJournal();
    rfRenderJournalStats();
  });
  document.getElementById('rfJournalModal').classList.add('open');
};

window.closeJournal = function() {
  if (_unsubJournal) { _unsubJournal(); _unsubJournal = null; }
  document.getElementById('rfJournalModal').classList.remove('open');
};

// ── Toggle TP blocks in form ──
window.rfToggleTP = function(n) {
  const idx = n - 1;
  _rfTpEnabled[idx] = !_rfTpEnabled[idx];
  const btn   = document.getElementById('jf-toggle-tp' + n);
  const block = document.getElementById('jf-tp-block-' + n);
  if (_rfTpEnabled[idx]) {
    if (btn)   { btn.classList.add('active'); btn.textContent = '✕ TP' + n; }
    if (block) block.style.display = 'block';
    if (n < 3) {
      const next = document.getElementById('jf-toggle-tp' + (n + 1));
      if (next) next.disabled = false;
    }
  } else {
    if (btn)   { btn.classList.remove('active'); btn.textContent = '+ TP' + n; }
    if (block) block.style.display = 'none';
    for (let i = n; i < 3; i++) {
      _rfTpEnabled[i] = false;
      const b = document.getElementById('jf-toggle-tp' + (i + 1));
      const bl = document.getElementById('jf-tp-block-' + (i + 1));
      if (b)  { b.classList.remove('active'); b.textContent = '+ TP' + (i + 1); b.disabled = true; }
...
window.rfCalcJournalPnl = function() {
  ...
  const calcRow = document.getElementById('jf-calc-row');
  ...
  for (let i = 0; i < 3; i++) {
    ...
    if (isNaN(tpPrice) || isNaN(tpPct) || tpPct <= 0) break;
  ...
  } else if (remainingSize > 0) { return; }

  if (totalWeight === 0) return;
  const avgRR = weightedRR / totalWeight;
  ...
  const rrEl  = document.getElementById('jf-calc-rr');
  const outEl = document.getElementById('jf-calc-outcome');
  if (pnlEl) { pnlEl.textContent = rfFmtPnl(totalPnl); pnlEl.style.color = totalPnl >= 0 ? 'var(--green)' : 'var(--red)'; }
  if (rrEl)  rrEl.textContent = avgRR.toFixed(2) + ':1';
  if (outEl) { outEl.textContent = outcome === 'win' ? '✅ Win' : outcome === 'loss' ? '❌ Loss' : '➖ BE'; outEl.style.color = outcome === 'win' ? 'var(--green)' : outcome === 'loss' ? 'var(--red)' : 'var(--accent)'; }
};
...
window.saveJournalEntry = async function() {
  ...
  const date   = document.getElementById('jf-date')?.value || new Date().toISOString().split('T')[0];
  const symbol = (document.getElementById('jf-symbol')?.value || '').trim().toUpperCase();
  const dir    = document.getElementById('jf-dir')?.value || 'long';
  ...
  if (!symbol)                         { notify('Inserisci il Pair', 'err'); if (btn) { btn.disabled=false; btn.textContent='+ Aggiungi'; } return; }
  if (isNaN(entryP)||isNaN(slP)||isNaN(size)||size<=0) { notify('Inserisci Entry, SL e Size', 'err'); if (btn) { btn.disabled=false; btn.textContent='+ Aggiungi'; } return; }
  if (isNaN(exitP))                    { notify('Inserisci il prezzo di uscita', 'err'); if (btn) { btn.disabled=false; btn.textContent='+ Aggiungi'; } return; }
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const tpPrice = parseFloat(document.getElementById('jf-tp' + n + '-price')?.value);
    const tpPct   = parseFloat(document.getElementById('jf-tp' + n + '-pct')?.value);
    if (isNaN(tpPrice)||isNaN(tpPct)||tpPct<=0) { notify('Compila prezzo e % per TP'+n, 'err'); if (btn) { btn.disabled=false; btn.textContent='+ Aggiungi'; } return; }
    const closed = remainingSize * (tpPct / 100);
  ...
  const entryObj = {
    id: Date.now(), date, symbol, dir, outcome,
    pnl: parseFloat(totalPnl.toFixed(2)), rr: avgRR,
    setup, tf, notes, mood,
    entryPrice: entryP, slPrice: slP, exitPrice: exitP, size, tps,
    exchange: window._activeExchange || 'manual',
    screenshot: ''
  };

  try {
    await setDoc(doc(db, 'journals', currentUser.uid), { entries: [entryObj, ...rfJournalEntries] });
    // Reset form
    ...
    const dirEl = document.getElementById('jf-dir'); if (dirEl) dirEl.value = 'long';
    const tfEl  = document.getElementById('jf-tf');  if (tfEl)  tfEl.value  = '';
    const moodEl= document.getElementById('jf-mood'); if (moodEl) moodEl.value = '';
    const calcRow = document.getElementById('jf-calc-row'); if (calcRow) calcRow.style.display = 'none';
    _rfTpEnabled = [false, false, false];
    [1, 2, 3].forEach(n => {
      const b = document.getElementById('jf-toggle-tp' + n);
      const bl = document.getElementById('jf-tp-block-' + n);
      if (b)  { b.classList.remove('active'); b.textContent = '+ TP' + n; b.disabled = n > 1; }
      if (bl) bl.style.display = 'none';
    });
    notify('Trade aggiunto al Journal ✓', 'ok');
  } catch(e) {
    notify('Errore: ' + e.message, 'err');
  }
...
// ── Delete entry ──
window.rfDeleteJournalEntry = async function(id) {
  if (!currentUser) return;
  if (!confirm('Eliminare questo trade dal journal?')) return;
  try {
    await setDoc(doc(db, 'journals', currentUser.uid), { entries: rfJournalEntries.filter(e => e.id !== id) });
  } catch(e) { notify('Errore: ' + e.message, 'err'); }
};

// ── Edit entry ──
window.rfEditJournalEntry = function(id) {
  const e = rfJournalEntries.find(x => x.id === id);
  if (!e) return;
  ...
  const msg = document.getElementById('ej-msg'); if (msg) msg.textContent = '';
  document.getElementById('rfJournalEditModal').classList.add('open');
};

window.closeEditJournal = function() {
  document.getElementById('rfJournalEditModal').classList.remove('open');
};
...
window.saveEditJournal = async function() {
  ...
  if (msg) { msg.textContent = 'Salvataggio...'; msg.style.color = 'var(--muted2)'; }
  const pnl = parseFloat(document.getElementById('ej-pnl').value);
  const updated = rfJournalEntries.map(e => {
    if (e.id !== id) return e;
    return {
      ...e,
      ...
      outcome:    document.getElementById('ej-outcome').value,
  ...
  try {
    ...
    if (msg) { msg.textContent = '✓ Salvato!'; msg.style.color = 'var(--green)'; }
    setTimeout(() => closeEditJournal(), 700);
  } catch(err) {
    if (msg) { msg.textContent = 'Errore: ' + err.message; msg.style.color = 'var(--red)'; }
  }
};

// ── Reset journal ──
window.rfConfirmResetJournal = async function() {
  if (!currentUser) return;
  ...
  try {
    ...
    notify('Journal resettato', 'ok');
  } catch(e) { notify('Errore: ' + e.message, 'err'); }
};

// ── Import from exchange ──
window.rfImportExchangeToJournal = async function() {
  if (!currentUser) { notify('Devi essere loggato', 'err'); return; }
  const ex = window._activeExchange || 'bitget';
  const btn = document.getElementById('jf-import-btn');
  ...
  const isBybit = ex === 'bybit' || ex === 'bybit_demo';
  const lsKey   = isWeex ? 'weex_api_key' : isBingx ? 'bingx_api_key' : isBybit ? 'bybit_api_key' : 'bitget_api_key';
  const apiKey  = localStorage.getItem(lsKey);
  if (!apiKey) {
    notify('API ' + ex + ' non configurate. Imposta le chiavi prima.', 'err');
    return;
  }
  ...
  try {
    let fills = [];
    const PROXY_BITGET = 'https://bitget-proxy-mze2.onrender.com';
    const PROXY_BYBIT  = 'https://bybit-proxy-2ggw.onrender.com';
    const PROXY_WEEX   = 'https://weex-proxy.onrender.com';
    const PROXY_BINGX  = 'https://bingx-proxy.onrender.com';

    // helper per fetch con autenticazione (usa i proxy già configurati in riskflow.js)
    if (ex === 'bitget') {
      const { apiKey: k, secret: s, passphrase: p } = loadBitgetKeys();
      const qstr = new URLSearchParams({ endpoint: '/api/v2/mix/order/fills', productType: 'USDT-FUTURES', limit: '100' }).toString();
      const res = await fetch(`${PROXY_BITGET}?${qstr}`, { headers: { 'x-bitget-key': k, 'x-bitget-secret': s, 'x-bitget-passphrase': p || '' } });
      const data = await res.json();
      fills = data.data?.fillList || data.data?.orderList || data.data || [];
    } else if (ex === 'bybit' || ex === 'bybit_demo') {
      const { apiKey: k, secret: s } = loadBybitKeys();
      const ts = String(Date.now());
      const qstr = new URLSearchParams({ endpoint: '/v5/execution/list', category: 'linear', limit: '100' }).toString();
      const res = await fetch(`${PROXY_BYBIT}?${qstr}`, { headers: { 'x-bybit-key': k, 'x-bybit-secret': s, 'x-bybit-timestamp': ts } });
      const data = await res.json();
      fills = data.result?.list || [];
    } else if (ex === 'weex') {
      const { apiKey: k, secret: s, passphrase: p } = loadWeexKeys();
      const qstr = new URLSearchParams({ endpoint: '/capi/v3/userTrades', productType: 'USDT-FUTURES', limit: '100' }).toString();
      const res = await fetch(`${PROXY_WEEX}?${qstr}`, { headers: { 'x-weex-key': k, 'x-weex-secret': s, 'x-weex-passphrase': p || '' } });
      const data = await res.json();
      fills = data.data || [];
    } else if (ex === 'bingx') {
      const { apiKey: k, secret: s } = loadBingxKeys();
      const qstr = new URLSearchParams({ endpoint: '/openApi/swap/v2/trade/allFillOrders', limit: '100' }).toString();
      const res = await fetch(`${PROXY_BINGX}?${qstr}`, { headers: { 'x-bingx-key': k, 'x-bingx-secret': s } });
      const data = await res.json();
      fills = data.data?.list || data.data || [];
    }

    if (!fills.length) { notify('Nessun fill trovato per ' + ex, 'err'); return; }

    // IDs già presenti nel journal
    const existingIds = new Set(rfJournalEntries.filter(e => e.exchangeOrderId).map(e => e.exchangeOrderId));

    // Raggruppa per orderId e converti in journal entries
    const orderMap = new Map();
    for (const f of fills) {
      const side = (f.tradeSide || f.side || f.execType || '').toLowerCase();
      // Considera solo chiusure
      const isClose = side.includes('close') || side === 'sell' || side === 'buy_to_cover'
        || side === 'closelong' || side === 'closeshort' || f.closedSize;
      if (!isClose && ex !== 'bybit' && ex !== 'bybit_demo') continue;

      const pnl = parseFloat(f.profit || f.pnl || f.realizedPnl || f.closedPnl || 0);
      const orderId = String(f.orderId || f.fillId || f.execId || f.id || Date.now());
      if (existingIds.has(orderId)) continue;

      if (!orderMap.has(orderId)) orderMap.set(orderId, []);
      orderMap.get(orderId).push({ ...f, _pnl: pnl });
    }

    if (orderMap.size === 0) { notify('Nessun nuovo trade da importare', 'ok'); return; }
    ...
    for (const [orderId, group] of orderMap) {
      ...
      const ts   = first.cTime || first.createTime || first.execTime || first.createdTime || Date.now();
      const d    = new Date(parseInt(ts));
      const date = d.toISOString().split('T')[0];
      const rawSym = first.symbol || '';
      const sym  = rawSym.replace('_UMCBL','').replace('_DMCBL','').replace('USDT','').replace('-USDT','') + 'USDT';
      const tradeSide = (first.tradeSide || first.holdSide || first.side || '').toLowerCase();
      const dir  = (tradeSide.includes('long') || tradeSide.includes('buy')) ? 'long' : 'short';
      const entryPrice = parseFloat(first.openPriceAvg || first.averagePrice || first.avgPrice || first.price || 0);
      const exitPrice  = parseFloat(first.fillPrice || first.execPrice || first.price || 0);
      const size       = parseFloat(first.baseVolume || first.qty || first.filledQty || first.size || 0);

      newEntries.push({
        id: Date.now() + Math.random(),
        date, symbol: sym, dir, outcome,
        pnl: parseFloat(totalPnl.toFixed(2)),
        rr: null, setup: '', tf: '', notes: '', mood: '',
        entryPrice, exitPrice, slPrice: 0, size, tps: [],
        exchange: ex,
    ...
    if (!newEntries.length) { notify('Nessun nuovo trade da importare', 'ok'); return; }
    await setDoc(doc(db, 'journals', currentUser.uid), { entries: [...newEntries, ...rfJournalEntries] });
    notify(`✓ Importati ${newEntries.length} trade da ${ex}`, 'ok');
  } catch(e) {
    console.error('rfImportExchangeToJournal:', e);
    notify('Errore importazione: ' + e.message, 'err');
  } finally {
    if (btn) { btn.disabled = false; btn.textContent = '⬇ Importa ' + (ex.charAt(0).toUpperCase() + ex.slice(1).replace('_demo',' Demo')); }
  }
};

// ── Update import button label based on active exchange ──
function rfUpdateJournalImportBtn() {
  const btn = document.getElementById('jf-import-btn');
  if (!btn) return;
  const ex = window._activeExchange || 'bitget';
  const label = ex === 'bybit_demo' ? 'Bybit Demo' : ex.charAt(0).toUpperCase() + ex.slice(1);
  btn.textContent = '⬇ Importa ' + label;
}

// ── Render journal stats bar ──
function rfRenderJournalStats() {
  const entries = rfJournalEntries;
  const total   = entries.length;
  const wins    = entries.filter(e => e.outcome === 'win').length;
  const losses  = entries.filter(e => e.outcome === 'loss').length;
  const totalPnl = entries.reduce((a, e) => a + (e.pnl || 0), 0);
  const best     = total > 0 ? Math.max(...entries.map(e => e.pnl || 0)) : null;
  ...
  const set = (id, txt, cls) => {
    ...
    el.textContent = txt;
    if (cls) el.className = el.className.replace(/pos|neg|neutral/g, '').trim() + ' ' + cls;
  };
  set('jst-total', total);
  set('jst-wr',    wrPct !== null ? wrPct + '%' : '—', wrPct !== null ? (wrPct >= 50 ? 'pos' : 'neg') : 'neutral');
  set('jst-pnl',   total > 0 ? rfFmtPnl(totalPnl) : '$0', totalPnl >= 0 ? 'pos' : 'neg');
  set('jst-best',  best !== null ? rfFmtPnl(best) : '—', best !== null ? (best >= 0 ? 'pos' : 'neg') : '');
  set('jst-rr',    avgRR ? avgRR + ':1' : '—');

  // Cooldown banner: 2+ consecutive losses
  const lastTwo = entries.slice(0, 2);
  const cooldownEl = document.getElementById('jf-cooldown');
  if (cooldownEl) cooldownEl.style.display = (lastTwo.length >= 2 && lastTwo.every(e => e.outcome === 'loss')) ? 'block' : 'none';

  // Overtrading banner: >5 trades today
  const today = new Date().toISOString().split('T')[0];
  const todayCount = entries.filter(e => e.date === today).length;
  const overtradingEl = document.getElementById('jf-overtrading');
  if (overtradingEl) overtradingEl.style.display = todayCount > 5 ? 'block' : 'none';
}

// ── Render journal table ──
window.rfRenderJournal = function() {
  rfRenderJournalStats();
  const entries = rfJournalEntries;
  const tbody   = document.getElementById('jf-tbody');
  ...
  const iconEdit = `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>`;
  const iconDel  = `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>`;

  tbody.innerHTML = entries.map((e, i) => {
    const dl  = e.dir === 'long' ? 'Long' : 'Short';
    const ol  = e.outcome === 'win' ? 'Win' : e.outcome === 'loss' ? 'Loss' : 'BE';
    const nt  = e.notes ? (e.notes.length > 22 ? e.notes.substring(0, 22) + '…' : e.notes) : '—';
    const pnlColor = (e.pnl || 0) >= 0 ? 'var(--green)' : 'var(--red)';
    const dirColor = e.dir === 'long' ? 'var(--green)' : 'var(--red)';
    const outColor = e.outcome === 'win' ? 'var(--green)' : e.outcome === 'loss' ? 'var(--red)' : 'var(--accent)';
    const exTag    = e.exchange && e.exchange !== 'manual'
      ? `<span style="font-size:8px;padding:1px 4px;border-radius:2px;background:var(--accent-dim);color:var(--accent);margin-left:3px">${e.exchange}</span>` : '';

    return `<tr>
      <td style="color:var(--muted2);font-size:10px">${entries.length - i}</td>
      <td style="color:var(--muted2);white-space:nowrap;font-size:10px">${e.date || '—'}</td>
      <td style="font-weight:500">${e.symbol || '—'}${exTag}</td>
      <td><span style="background:${e.dir==='long'?'var(--green-dim)':'var(--red-dim)'};color:${dirColor};padding:2px 6px;border-radius:3px;font-size:9px;font-weight:600;text-transform:uppercase">${dl}</span></td>
      <td><span style="background:${e.outcome==='win'?'var(--green-dim)':e.outcome==='loss'?'var(--red-dim)':'var(--accent-dim)'};color:${outColor};padding:2px 6px;border-radius:3px;font-size:9px;font-weight:600;text-transform:uppercase">${ol}</span></td>
      <td style="color:${pnlColor};font-weight:700;font-family:'Syne',sans-serif">${rfFmtPnl(e.pnl||0)}</td>
      <td style="color:var(--muted2)">${e.rr ? e.rr + ':1' : '—'}</td>
      <td style="color:var(--muted2);font-size:10px;white-space:nowrap">${e.entryPrice||'—'} / ${e.exitPrice||'—'}</td>
      <td style="color:var(--muted2)">${e.setup || '—'}</td>
      <td style="color:var(--muted2)">${e.tf || '—'}</td>
      <td style="color:var(--muted2);font-size:10px" title="${e.notes||''}">${nt}</td>
      <td style="text-align:right;white-space:nowrap">
        <div style="display:flex;gap:4px;justify-content:flex-end">
          <button onclick="rfEditJournalEntry(${e.id})" title="Modifica" style="background:var(--surface2);border:1px solid var(--border);color:var(--muted2);border-radius:4px;width:26px;height:26px;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:.12s" onmouseover="this.style.color='var(--accent)';this.style.borderColor='var(--accent)'" onmouseout="this.style.color='var(--muted2)';this.style.borderColor='var(--border)'">${iconEdit}</button>
          <button onclick="rfDeleteJournalEntry(${e.id})" title="Elimina" style="background:var(--surface2);border:1px solid var(--border);color:var(--muted2);border-radius:4px;width:26px;height:26px;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:.12s" onmouseover="this.style.color='var(--red)';this.style.borderColor='var(--red)'" onmouseout="this.style.color='var(--muted2)';this.style.borderColor='var(--border)'">${iconDel}</button>
        </div>
      </td>
    </tr>`;
  }).join('');
...
// ── Update import button label when exchange changes ──
// Hook into rfSelectExchange by patching the existing function reference
const _rfSelectExchangeOrig = window.rfSelectExchange;
window.rfSelectExchange = function(ex) {
  if (typeof _rfSelectExchangeOrig === 'function') _rfSelectExchangeOrig(ex);
  rfUpdateJournalImportBtn();
};

// ── Init import button on page load ──
document.addEventListener('DOMContentLoaded', rfUpdateJournalImportBtn);
