...
function dllToggleSwitch() {
  ...
  const d = dllLoad();
  d.enabled = !d.enabled;
  // Al momento dell'attivazione, salva snapshot del balance attuale
  if (d.enabled && S.balance > 0) {
    d.snapshot = S.balance;
  }
...
function dllCheckBalance(currentBalance) {
  ...
  if (!d.snapshot || d.snapshot <= 0) {
    // inizializza snapshot se mancante
    d.snapshot = currentBalance;
    dllSave(d);
    return;
  }
...
function dllStartCountdown() {
  ...
  _dllTimer = setInterval(() => {
    ...
    if (rem <= 0) {
      ...
      _dllTimer = null;
      // Reset snapshot per il nuovo giorno — triggered_at azzerato, balance snapshot aggiornato
      const d = dllLoad();
      d.triggeredAt = 0;
      d.snapshot    = S.balance > 0 ? S.balance : 0;
      dllSave(d);
      dllRenderUI();
...
(async () => {
  ...
  async function fetchBitgetDashboard() {
    ...
    try {
      ...
      S.balance = equity;
      dllCheckBalance(equity); // Daily Loss Limit check
