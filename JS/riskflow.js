...
async function executeOrder(){
  ...
  if (isBingx) {
    ...
    try {
      ...
      // BingX: stopLoss/takeProfit vanno come stringhe JSON nell'order body
      // (BingX li vuole come stringhe che contengono JSON, non oggetti annidati)
      if (sl && sl > 0)      orderBody.stopLoss   = JSON.stringify({ type: 'STOP_MARKET',       stopPrice: fmtPrice_(sl),        price: '0', workingType: 'MARK_PRICE' });
      if (tpList.length > 0) orderBody.takeProfit = JSON.stringify({ type: 'TAKE_PROFIT_MARKET', stopPrice: fmtPrice_(tpList[0]), price: '0', workingType: 'MARK_PRICE' });
      await window._bingxRequest('/openApi/swap/v2/trade/order', {}, { method:'POST', body:JSON.stringify(orderBody) });
      notify('✓ Ordine inviato su BingX!','ok');
      clearAll();
      setTimeout(()=>window.refreshAccount(), 2000);
    } catch(e){
      notify('✗ Errore ordine: '+e.message,'err');
      console.error('executeOrder bingx error:', e);
    }
...
(async () => {
  ...
  async function bingxRequest(endpoint, params={}, options={}) {
    ...
    if (method === 'GET') {
      ...
      // POST: manda i params nel body JSON — il proxy li legge da req.body e li firma
      // (necessario per supportare valori complessi come stopLoss/takeProfit che sono stringhe JSON)
      const bodyParams = options.body ? JSON.parse(options.body) : {};
      url = `${BINGX_PROXY}?endpoint=${encodeURIComponent(endpoint)}`;
      fetchOpts = {method:'POST', headers: {...bingxHeaders, 'Content-Type': 'application/json'}, body: JSON.stringify(bodyParams)};
    }
    const res = await fetch(url, fetchOpts);
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
