...
async function executeOrderBingX(){
  ...
  try {
    ...
    const orderBody = {
      ...
      ...(sl ? {stopLoss:   {type:'MARK_PRICE', stopPrice: String(roundToTick(sl, cInfo.pricePlace))}} : {}),
      ...(tpList.length>0 ? {takeProfit: {type:'MARK_PRICE', stopPrice: String(roundToTick(tpList[0], cInfo.pricePlace))}} : {}),
    };
...
(async () => {
  ...
  async function bingxRequest(endpoint, params={}, options={}) {
    ...
    if (method==='GET') {
      ...
      fetchOpts = {method:'GET', headers:{'Content-Type':'application/json','x-bingx-key':apiKey,'x-bingx-secret':secret}};
    } else {
      const qstr = Object.keys(params).length ? '&'+new URLSearchParams(params).toString() : '';
      url = `${BINGX_PROXY}?endpoint=${encodeURIComponent(endpoint)}${qstr}`;
  ...
  window.modifyBingxPositionSL = async function(idx) {
    ...
    try {
      await bingxRequest('/openApi/swap/v2/trade/positionMargin', {}, {
        method:'POST',
        body: JSON.stringify({ symbol:bxSym, stopLoss:{ type:'MARK_PRICE', stopPrice: String(newSL) }, positionSide: side==='long'?'LONG':'SHORT' }),
      });
  ...
  window.modifyBingxPositionTP = async function(posIdx, tpN) {
    ...
    try {
      await bingxRequest('/openApi/swap/v2/trade/positionMargin', {}, {
        method:'POST',
        body: JSON.stringify({ symbol:bxSym, takeProfit:{ type:'MARK_PRICE', stopPrice: String(newTP) }, positionSide: side==='long'?'LONG':'SHORT' }),
      });
  ...
  window.moveBingxToBreakeven = async function(idx) {
    ...
    try {
      await bingxRequest('/openApi/swap/v2/trade/positionMargin', {}, {
        method:'POST',
        body: JSON.stringify({ symbol:bxSym, stopLoss:{ type:'MARK_PRICE', stopPrice: String(entry) }, positionSide: side==='long'?'LONG':'SHORT' }),
      });
      const inp = document.getElementById('pos-sl-input-'+idx);
  ...
  window.rfSaveApiKeys = async function() {
    ...
    try {
      ...
      setTimeout(()=>fetchActiveDashboard(), 500);
