...
// Valore "originale" dello SL al momento in cui è stato impostato.
// Viene resettato da clearAll() e aggiornato quando si imposta un nuovo SL.
let _slpRefPrice = null;
...
// Ritorna true se il toggle è bloccato (attivo + SL impostato = sei in posizione)
function slpIsLocked() {
  return slpLoad() && _slpRefPrice !== null;
}
...
function slpRenderUI() {
  ...
  wrap.title = locked
    ? '🛡 SL Shield attivo — non puoi disattivarlo mentre sei in posizione (Clear per uscire)'
    : "SL Shield: impedisce di allargare lo stop loss dopo l'entrata";
...
function slpToggle() {
  if (slpIsLocked()) {
    notify('🛡 SL Shield bloccato — non puoi disattivarlo mentre sei in posizione. Premi Clear per uscire.', 'err');
    return;
  }
  const on = !slpLoad();
  slpSave(on);
  ...
  if (on) {
    ...
    _slpRefPrice = (sl && sl > 0) ? sl : null;
    notify('🛡 SL Shield ON — lo SL non può essere allargato', 'ok');
    slpRenderUI();
  } else {
    _slpRefPrice = null;
    notify('SL Shield disattivato', '');
  }
}

/**
 * Controlla se il nuovo SL proposto è ammesso dal SL Protect.
 * Ritorna true se OK (si può applicare), false se va rifiutato.
 * @param {number} newSL  - nuovo prezzo SL proposto
 * @param {boolean} silent - se true, non mostra notify
 */
function slpCheck(newSL, silent = false) {
  if (!slpLoad()) return true;         // feature spenta → sempre ok
  if (!_slpRefPrice) return true;      // nessun riferimento ancora → ok
  if (!newSL || newSL <= 0) return true;

  const entry = S.orderType === 'market'
    ? S.lastPrice
    : parseFloat(document.getElementById('entryVal')?.value);

  if (!entry || entry <= 0) return true; // entry non impostata → ok

  const isLong  = entry > _slpRefPrice;  // SL sotto entry = long
  const isShort = entry < _slpRefPrice;  // SL sopra entry = short

  if (isLong) {
    // LONG: lo SL di riferimento è sotto entry. Non si può scendere ulteriormente.
    if (newSL < _slpRefPrice) {
      if (!silent) notify(`🛡 SL Shield: non puoi allargare lo SL (min $${fmtPrice(_slpRefPrice)})`, 'err');
      return false;
    }
  } else if (isShort) {
    // SHORT: lo SL di riferimento è sopra entry. Non si può salire ulteriormente.
    if (newSL > _slpRefPrice) {
      if (!silent) notify(`🛡 SL Shield: non puoi allargare lo SL (max $${fmtPrice(_slpRefPrice)})`, 'err');
      return false;
    }
  }
  return true;
...
/**
 * Aggiorna il prezzo di riferimento dello SL Protect.
 * Va chiamato ogni volta che l'utente imposta un nuovo SL valido.
 * Solo se il nuovo SL è "più stretto" dell'attuale riferimento (o non c'è riferimento).
 */
function slpUpdateRef(newSL) {
  if (!newSL || newSL <= 0) return;
  if (!_slpRefPrice) {
    _slpRefPrice = newSL;
    slpRenderUI(); // aggiorna il lucchetto: ora siamo "in posizione"
    return;
  }
  const entry = S.orderType === 'market'
    ? S.lastPrice
    : parseFloat(document.getElementById('entryVal')?.value);
  if (!entry || entry <= 0) { _slpRefPrice = newSL; slpRenderUI(); return; }

  const isLong = entry > _slpRefPrice;
  if (isLong) {
    if (newSL > _slpRefPrice) { _slpRefPrice = newSL; slpRenderUI(); }
  } else {
    if (newSL < _slpRefPrice) { _slpRefPrice = newSL; slpRenderUI(); }
  }
}
...
function syncLine(type){
  ...
  // SL Protect: quando l'utente digita nel campo SL, verifica
  if(type === 'sl' && v && !slpCheck(v)){
    // Ripristina al valore di riferimento e tronca l'input
    el.value = fmtPrice(_slpRefPrice);
    if(_slpRefPrice) setChartLine('sl', _slpRefPrice);
    drawCanvas();
    return;
  }
  if(type === 'sl' && v) slpUpdateRef(v);
...
document.addEventListener('mousemove', e=>{
  ...
  if(dragState.type.startsWith('ld')){
    ...
    // ── SL Protect check durante il drag ──
    if(dragState.type === 'sl' && !slpCheck(price, true)){
      // Blocca: rimette il prezzo al valore di riferimento (o all'attuale consentito)
      const clampedSL = _slpRefPrice || price;
      DRAG_PRICES['sl'] = clampedSL;
      const el=document.getElementById('slVal');
      if(el) el.value = fmtPrice(clampedSL);
      setChartLine('sl', clampedSL);
      drawCanvas();
      return;
    }
...
document.addEventListener('mouseup', e=>{
  ...
  if(dragState){
    ...
    if(t.startsWith('ld')){
      ...
      // SL Protect: aggiorna riferimento al nuovo SL (solo se è più stretto)
      if(t === 'sl' && DRAG_PRICES['sl']) slpUpdateRef(DRAG_PRICES['sl']);
...
function setField(type,price){
  const idMap={entry:'entryVal',sl:'slVal',tp1:'tp1',tp2:'tp2',tp3:'tp3'};
  // SL Protect: blocca se il nuovo SL allarga la distanza
  if(type === 'sl' && !slpCheck(price)) return;
  document.getElementById(idMap[type]).value=fmtPrice(price);
  if(type.startsWith('tp')&&!S.tpEnabled) toggleTP();
  // SL Protect: aggiorna riferimento (se il SL è più stretto, il ref avanza)
  if(type === 'sl') slpUpdateRef(price);
...
function clearAll(){
  ...
  document.getElementById('chartCalcBtn').classList.remove('visible');
  _slpRefPrice = null; // reset SL Protect reference on clear
  slpRenderUI();        // sblocca il toggle (fuori posizione)
  drawCanvas(); calc();
}
