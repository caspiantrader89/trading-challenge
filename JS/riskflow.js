...
async function loadCandles(symbol,tf){
  ...
  if(data&&data.length){
    ...
    S._lastCandleTime=null;  // forza reset OHLC al primo tick del nuovo pair
    updatePriceDisp(last.close,data[data.length-2]?.close||last.open);
    // Non chiamare fitContent() qui: la price scale (asse Y) è ancora quella
    // del pair precedente. Il fitContent() viene fatto dal primo tick in startTick(),
    // dopo che candleSeries.update() ha aggiunto la candela live corretta.
    startTick(symbol);
...
(async () => {
  ...
  // Inizializza UI exchange al caricamento del modal
  const _initExchangeUI = () => {
    window.rfSelectExchange(_activeExchange);
  };
