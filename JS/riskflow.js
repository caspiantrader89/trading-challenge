...
async function loadBybitPairs() {
  try {
    const r = await fetch(`${BYBIT_BASE}/v5/market/instruments-info?category=linear&limit=1000`);
    const j = await r.json();
...
async function fetchBybitCandles(symbol, tf) {
  ...
  try {
    ...
    for (let i = 0; i < MAX_CALLS; i++) {
      const url = `${BYBIT_BASE}/v5/market/kline?category=linear&symbol=${sym}&interval=${interval}&end=${endTime}&limit=${PER_CALL}`;
...
async function fetchBybitTicker(symbol) {
  ...
  try {
    ...
    let r;
    try { r = await fetch(`${BYBIT_BASE}/v5/market/tickers?category=linear&symbol=${sym}`, {signal: ctrl.signal}); }
...
async function fetchBybitContractInfo(symbol) {
  const sym = symbol.endsWith('USDT') ? symbol : symbol + 'USDT';
  try {
    const r = await fetch(`${BYBIT_BASE}/v5/market/instruments-info?category=linear&symbol=${sym}`);
    const j = await r.json();
    const d = j.result?.list?.[0];
...
(async () => {
  ...
  async function saveKeysToFirestore(uid, apiKey, secret, passphrase, exchange) {
    ...
    await setDoc(doc(db,'apiKeys',uid), {exchange: exchange||'bitget',encKey,encSecret,encPass,updatedAt:Date.now()});
  }
  ...
  async function deleteKeysFromFirestore(uid) {
    await setDoc(doc(db,'apiKeys',uid), {exchange:_activeExchange||'bitget',encKey:'',encSecret:'',encPass:'',updatedAt:Date.now()});
  }
  ...
  async function syncKeysFromFirestore(uid) {
    ...
    const ex = keys.exchange||'bitget';
    // Ripristina l'exchange salvato
    if (ex && ['bitget','bybit','bybit_demo'].includes(ex)) {
      _activeExchange = ex;
  ...
  async function bybitRequest(endpoint, params={}, options={}) {
    ...
    const method = options.method||'GET';
    let url, fetchOpts;
    const demoHeader = isDemo ? {'x-bybit-demo':'true'} : {};
    if (method === 'GET') {
      const qstr = new URLSearchParams({endpoint,...params}).toString();
      url = `${BITGET_PROXY}?${qstr}`;
      fetchOpts = {method:'GET', headers:{'x-bybit-key':apiKey,'x-bybit-secret':secret,...demoHeader}};
    } else {
      url = `${BITGET_PROXY}?endpoint=${encodeURIComponent(endpoint)}`;
      fetchOpts = {method:'POST', headers:{'x-bybit-key':apiKey,'x-bybit-secret':secret,...demoHeader,'Content-Type':'application/json'}, body:options.body||'{}'};
    }
    const res = await fetch(url, fetchOpts);
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = await res.json();
  ...
  async function fetchBitgetDashboard() {
    ...
    try {
      ...
      if (isBybit) {
        // ── Bybit balance ──
        const balData = await bybitRequest('/v5/account/wallet-balance', {accountType:'CONTRACT'});
  ...
  window.rfLoadApiKeysUI = async function() {
    ...
    const localSec = localStorage.getItem(lsSecret);
    if (localKey) {
      document.getElementById('apiKeyInput').value = '••••••••••••••••';
      document.getElementById('apiSecretInput').value = '••••••••••••••••';
