...
async function executeOrder(){
  ...
  if (isBingx) {
    ...
    try {
      ...
      // BingX richiede stopLoss/takeProfit come oggetti JSON, non stringhe
  ...
  async function fetchBitgetDashboard() {
    ...
    try {
      ...
      if (isBingx) {
        ...
        console.log('[BingX balance raw]', JSON.stringify(balData));
        // v3 risponde con data.balance oppure data direttamente
        const usdt = balData.data?.balance ?? balData.data ?? {};
        equity    = parseFloat(usdt.equity          || usdt.balance        || usdt.totalWalletBalance || 0);
        available = parseFloat(usdt.availableMargin || usdt.available      || usdt.availableBalance   || 0);
        upnl      = parseFloat(usdt.unrealizedProfit|| usdt.unrealizedPnl  || 0);
