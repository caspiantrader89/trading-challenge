...
async function loadCandles(symbol,tf){
  ...
  if(data&&data.length){
    ...
    // ── FIX: adatta subito l'asse Y al range di prezzo del nuovo pair ──
    // Senza questo, il chart resta "zoomato" sui prezzi del pair precedente
    // e il primo tick (dopo 1500ms) arriva troppo tardi.
...
function startTick(symbol){
  ...
  S.tickTimer=setInterval(async()=>{
    ...
    try{
      ...
      // Fix: fitContent + reset autoScale al primo tick per garantire
      // che l'asse Y si riadatti al range del nuovo pair dopo l'update live
      if(firstTick){
...
// ─── GOTO PRICE ───
function gotoPrice(){
  // ── FIX: forza reset dell'autoScale sull'asse Y prima di scrollare ──
  // Questo garantisce che il grafico si riadatti al range di prezzo del pair corrente,
  // non a quello del pair precedente o a un vecchio zoom manuale.
  chart.applyOptions({ rightPriceScale: { autoScale: true } });
  chart.timeScale().scrollToRealTime();
  chart.timeScale().fitContent();
}
