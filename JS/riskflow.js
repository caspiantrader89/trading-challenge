...
const FEE_DEFAULTS = {
  bitget:     0.0006,   // 0.06% taker
  bybit:      0.00055,  // 0.055% taker
  bybit_demo: 0.00055,
  weex:       0.0006,   // 0.06% taker
  bingx:      0.0005,   // 0.05% taker
};

function getFeeRate() {
  const ex = window._activeExchange || 'bitget';
  const saved = parseFloat(localStorage.getItem('rf_fee_' + ex));
  return (!isNaN(saved) && saved >= 0) ? saved : (FEE_DEFAULTS[ex] || 0.0006);
}

function calcFeeUsd(sizeUsdt) {
  return sizeUsdt * getFeeRate() * 2;
}

// size = riskUsd / (slDistPct/100 + feeRate*2)
function calcSizeWithFee(riskUsd, slDistPct) {
  const feeRate = getFeeRate();
  const denom = (slDistPct / 100) + (feeRate * 2);
  return denom > 0 ? riskUsd / denom : 0;
}
// Aggiorna il campo input fee nell'UI in base all'exchange attivo
function feeRenderUI() {
  const el = document.getElementById('feeRateInput');
  if (!el) return;
  const rate = getFeeRate();
  el.value = (rate * 100).toFixed(3);
}

// Salva la fee personalizzata nel localStorage per l'exchange attivo
function saveFeeRate() {
  ...
  if (isNaN(pct) || pct < 0) return;
  const ex = window._activeExchange || 'bitget';
  localStorage.setItem('rf_fee_' + ex, String(pct / 100));
  updateCalc(); // ricalcola subito size con nuova fee
}
...
function drawDragLine(type, price, y){
  ...
  if(isLadder){
    ...
    if(aIdx>=0 && sl && riskRaw){
      ...
      const slDist = Math.abs(price - sl);
      const slDistPct = slDist / price * 100;
      const size = slDistPct>0 ? calcSizeWithFee(orderRisk, slDistPct) : 0;
...
function calcLadder(){
  ...
  if(laddFeeEl) laddFeeEl.textContent = '$'+fmt(calcFeeUsd(totalSize));
...
function calc(){
  ...
  const riskUsd=S.riskMode==='pct'?(bal*riskRaw/100):riskRaw;
  const size=calcSizeWithFee(riskUsd, slDistPct);
  const feeUsd=calcFeeUsd(size);
  const margin=size/lev;
  ...
  if(feeEl) feeEl.textContent='$'+fmt(feeUsd)+' ('+(getFeeRate()*100).toFixed(3)+'% x2)';
...
async function openModal(){
  ...
  document.getElementById('modalRows').innerHTML=`
    ...
    <div class="mrow"><span class="ml">Fee est.</span><span class="mv" style="color:var(--muted2)">$${fmt(calcFeeUsd(parseFloat(size)))}</span></div>
    ${tpH}`;
...
async function openLadderModal(){
  ...
  document.getElementById('modalRows').innerHTML=`
    ...
    <div class="mrow"><span class="ml">Fee est.</span><span class="mv" style="color:var(--muted2)">$${fmt(calcFeeUsd(executableTotalSize))}</span></div>
    ${tpH}`;
  document.getElementById('modalBg').className='modal-bg open';
}
