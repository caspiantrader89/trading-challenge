  ...
  async function fetchBitgetDashboard() {
    ...
    try {
      ...
      if (isBingx) {
        ...
        // ── Abbina SL/TP dagli open orders alle posizioni ──
        try {
          const ordData = await bingxRequest('/openApi/swap/v2/trade/openOrders');
          const orders  = ordData?.data?.orders || [];
          for (const pos of positions) {
            const bsym = pos._bingxSymbol;
            const symOrders = orders.filter(o => o.symbol === bsym);
            const slOrder = symOrders.find(o => o.type === 'STOP_MARKET'        && o.positionSide === pos.holdSide.toUpperCase());
            const tpOrder = symOrders.find(o => o.type === 'TAKE_PROFIT_MARKET' && o.positionSide === pos.holdSide.toUpperCase());
            if (slOrder) pos.stopLoss   = parseFloat(slOrder.stopPrice || 0);
            if (tpOrder) pos.takeProfit = parseFloat(tpOrder.stopPrice || 0);
          }
        } catch(e) { console.warn('BingX SL/TP abbinamento:', e.message); }
