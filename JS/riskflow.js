...
function _drawPosSubLine(idx, lineType, price, y, upnl, entry, size, sym, isLong, W) {
  ...
  ctx.save();
  // NON ridisegniamo la linea tratteggiata: la priceLine LightweightCharts la gestisce già.
  // Aggiungiamo solo le label overlay canvas.

  const H = 20, R = 3, PAD = 7, GAP = 5;

  // calcola P&L a questo livello
  const pnlAtLevel = isSL
    ? (isLong ? (price - entry) : (entry - price)) * size
    : (isLong ? (price - entry) : (entry - price)) * size;
  const pnlStr = (pnlAtLevel >= 0 ? '+' : '') + '$' + fmt(Math.abs(pnlAtLevel));

  // Label: "[SL]  -$6.78 USD  · $0.0023"
  ...
  // [TYPE] pill
  const typePillW = typeLabelW + PAD * 2 + 4;
  ctx.fillStyle = lineColor;
  rrect(ctx, 0, pillY, typePillW, H, R); ctx.fill();
  ctx.fillStyle = '#fff';
  ctx.font = 'bold 10px "DM Mono",monospace';
  ctx.textAlign = 'left';
  ctx.textBaseline = 'middle';
  ctx.fillText(typeLabel, PAD, y);

  // PnL pill
  const pnlPillW = pnlW + PAD * 2;
  const pnlX = typePillW + GAP;
  ...
  // Prezzo pill (vicino all'asse destra)
  const pricePillW = priceW + PAD * 2;
  const priceX = W - pricePillW - 32; // lascia spazio per l'asse Y
  ctx.fillStyle = 'rgba(20,20,28,0.9)';
  rrect(ctx, priceX, pillY, pricePillW, H, R); ctx.fill();
  ctx.strokeStyle = lineColor;
  ctx.stroke();
  ctx.fillStyle = lineColor;
  ctx.fillText(priceStr, priceX + PAD, y);

  // ✕ button
  const xR = 8;
  const xX = priceX - xR * 2 - 6;
  const xY = y;
  ctx.beginPath();
  ...
  window._posLineZones[`${lineType}_x_${idx}`] = { x: xX - xR, y: xY - xR, w: xR * 2, h: xR * 2, idx, action: `remove_${lineType}` };
...
dc.addEventListener('mousedown', e=>{
  ...
  // ── controlla click su zone posizioni aperte (TP btn, SL btn, X) ──
...
dc.addEventListener('mousemove', e=>{
  if(!dragState){
    const rect0 = chartWrap.getBoundingClientRect();
    const cx = e.clientX - rect0.left, cy = e.clientY - rect0.top;
    // controlla hover su zone posizioni (TP btn, SL btn, X)
    if(window._posLineZones){
