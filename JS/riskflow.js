...
dc.addEventListener('mousemove', e=>{
  if(!dragState && !window._posDragState){
    ...
    if(!nearLine){
      // Nessuna linea vicina: togliamo temporaneamente pointer-events
      // così LightweightCharts riceve il vero evento nativo (crosshair + pan)
      dc.style.pointerEvents='none';
      // Usiamo requestAnimationFrame per rimetterlo dopo il frame,
      // così la chart ha il tempo di processare l'evento reale
      requestAnimationFrame(()=>{
        if(!dragState && !window._posDragState) dc.style.pointerEvents='all';
      });
    }
    return;
  }
...
dc.addEventListener('contextmenu', openCtxMenu);
