...
function getLadderWeights(n, prices, orders){
  ...
  if(orders && orders.length >= 1){
    ...
    if(hasCustom){
      // Usa customPct dove presente, distribuisce il resto equalmente
      const fixed = orders.map(o => o.customPct != null ? o.customPct : null);
      const fixedSum = fixed.reduce((a,b)=>a+(b||0),0);
      const autoCount = fixed.filter(v=>v===null).length;
      const autoVal = autoCount > 0 ? Math.max(0,(100-fixedSum)/autoCount) : 0;
      const result = fixed.map(v => v !== null ? v : autoVal);
      // Rinormalizza a 100
      const total = result.reduce((a,b)=>a+b,0);
      return result.map(v => Math.round(v/total*1000)/10);
    }
  }
  // Pesi scalari predefiniti: L1 parte da 10% e scala verso l'ultimo
  ...
  const preset = PRESET_WEIGHTS[n] || (() => {
    // fallback per n>4: crescita lineare da 10 a ~30
    const base = Array.from({length:n}, (_,i) => 10 + i*(20/(n-1)));
...
function renderLadderOrders(){
  ...
  // Pre-calcola i pesi di default per mostrarli subito come placeholder anche senza prezzi/SL
  ...
  for(let i=0;i<n;i++){
    ...
    const placeholder = i===0 ? 'Top' : i===n-1 ? 'Bottom' : 'prezzo';
    const customPct = order.customPct != null ? order.customPct : '';
    // Mostra il peso di default come placeholder per il campo %
    const pctPlaceholder = defaultWeights[i].toFixed(1);
    html += `
    <div class="ladd-order ${enabled?'active':'inactive'}" id="ladd-row-${i}">
      ...
      <div class="ladd-price-wrap" style="border-color:${LADDER_COLORS[i]}44">
        <input type="number" id="ladd-price-${i}" placeholder="${placeholder}" 
...
function calcLadder(){
  ...
  if(!activeOrders.length || !sl || !riskRaw){
    ...
    for(let i=0;i<n;i++){
      ...
      if(pc && pc.tagName==='INPUT') pc.placeholder='auto'; else if(pc) pc.textContent='—';
  ...
  // Pre-calcola pesi di default per ordini inattivi/senza prezzo
  const defaultWeightsCalc = getLadderWeights(n, [], LADDER.orders.slice(0,n));
  ...
  for(let i=0;i<n;i++){
    ...
    if(!od){
      // Mostra il peso di default anche se l'ordine non ha prezzo
      if(pc && document.activeElement !== pc){
        if(LADDER.orders[i].customPct == null){
          pc.placeholder = defaultWeightsCalc[i].toFixed(1);
          pc.value = '';
        }
      }
      if(sz) sz.textContent='— USDT';
