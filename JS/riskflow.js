...
async function loadCandles(symbol,tf){
  ...
  const data = isWeex ? await fetchWeexCandles(symbol,tf)
             : isBybit ? await fetchBybitCandles(symbol,tf)
             : await fetchCandles(symbol,tf);
  document.getElementById('chartLoading').classList.add('hidden');
  const exchLabel = isWeex ? 'Weex' : isBybit ? 'Bybit' : 'Bitget';
  if(data&&data.length){
...
function startTick(symbol){
  ...
  S.tickTimer=setInterval(async()=>{
    const isWeex  = window._activeExchange === 'weex';
    const isBybit = window._activeExchange === 'bybit' || window._activeExchange === 'bybit_demo';
    const t = isWeex ? await fetchWeexTicker(symbol)
  ...
  _bgPriceFetcher = setInterval(async () => {
    ...
    await Promise.all(syms.map(async sym => {
      try {
        const isWeex  = window._activeExchange === 'weex';
        const isBybit = window._activeExchange === 'bybit' || window._activeExchange === 'bybit_demo';
        const t = isWeex ? await fetchWeexTicker(sym)
...
(async () => {
  ...
  async function syncKeysFromFirestore(uid) {
    ...
    const exchanges = ['bitget', 'bybit', 'bybit_demo', 'weex'];
    ...
    try {
      ...
      if (rootSnap.exists()) {
        ...
        if (['bitget','bybit','bybit_demo','weex'].includes(lastEx)) {
  ...
  async function fetchBitgetOrders() {
    ...
    const {apiKey} = isWeex ? loadWeexKeys() : isBybit ? loadBybitKeys() : loadBitgetKeys();
    if (!apiKey) return;
    try {
      let orders = [];
      if (isWeex) {
  ...
  async function fetchBitgetDashboard() {
    ...
    const isWeex   = _activeExchange === 'weex';
    const isBybit  = _activeExchange === 'bybit' || _activeExchange === 'bybit_demo';
    const exchLabel= isWeex ? 'Weex' : isBybit ? (_activeExchange==='bybit_demo'?'Bybit Demo':'Bybit') : 'Bitget';
  ...
  function rfShowApp(username) {
    ...
    if(!_appInitDone){ _appInitDone=true;
      const isWeex  = _activeExchange === 'weex';
      const isBybit = _activeExchange === 'bybit' || _activeExchange === 'bybit_demo';
      (isWeex ? loadWeexPairs() : isBybit ? loadBybitPairs() : loadBitgetPairs()).then(()=>loadCandles(S.symbol, S.tf));
    }
  }
  ...
  window.rfSelectExchange = function(ex) {
    ...
    // Aggiorna tab attivi
    ['bitget','bybit','bybit_demo','weex'].forEach(e => {
      const idMap = {bitget:'exBtnBitget', bybit:'exBtnBybit', bybit_demo:'exBtnBybitDemo', weex:'exBtnWeex'};
      const btn = document.getElementById(idMap[e]);
      if(btn) btn.classList.toggle('active', e === ex);
    });
    // Passphrase: Bitget e Weex ne hanno bisogno
    ...
    if(keyInput) keyInput.placeholder = ex === 'weex' ? 'la tua API key Weex' : ex === 'bitget' ? 'la tua API key Bitget' : 'la tua API key Bybit';
    ...
    // Carica le chiavi del nuovo exchange
    ...
    if (_appInitDone) {
      ...
      (isWeexEx ? loadWeexPairs() : isBybitEx ? loadBybitPairs() : loadBitgetPairs()).then(() => loadCandles(S.symbol, S.tf));
    }
  };
  ...
  window.rfLoadApiKeysUI = async function() {
    ...
    const lsKey    = isWeex ? 'weex_api_key'    : isBybit ? 'bybit_api_key'    : 'bitget_api_key';
    const lsSecret = isWeex ? 'weex_api_secret'  : isBybit ? 'bybit_api_secret'  : 'bitget_api_secret';
    const localKey = localStorage.getItem(lsKey);
    if (localKey) {
      document.getElementById('apiKeyInput').value = '••••••••••••••••';
      document.getElementById('apiSecretInput').value = '••••••••••••••••';
      if (!isBybit) document.getElementById('apiPassInput').value = localStorage.getItem(isWeex ? 'weex_api_passphrase' : 'bitget_api_passphrase') ? '••••••••' : '';
      if (statusEl) { statusEl.textContent='✓ API keys caricate'; statusEl.className='api-status ok'; }
      return;
    }
    // Non in localStorage: prova da Firestore
    try {
      ...
      if (keys && keys.apiKey) {
        ...
        if (!isBybit && keys.passphrase) localStorage.setItem(isWeex ? 'weex_api_passphrase' : 'bitget_api_passphrase', keys.passphrase);
