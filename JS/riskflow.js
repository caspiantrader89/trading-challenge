  ...
  S.tickTimer=setInterval(async()=>{
    ...
    try{
      ...
      if(currentCandleOpen===null || S._lastCandleTime!==candleTime){
        // open = close candela precedente del NUOVO pair (impostato in loadCandles)
        currentCandleOpen = S._prevClose !== null ? S._prevClose : t.last;
      ...
      // (dopo l'update così la scala include anche la candela live)
      if(firstTick){
        chart.timeScale().fitContent();
        firstTick = false;
      }
